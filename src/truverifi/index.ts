...
export default class Truverifi extends VerificationClient {
	...
	async startVerification(): Promise<void> {
		const { data } = await this.http.post<
			{ phoneNumber: string } | TruverifiError
		>("/line/changeService", {
	...
	async claimVerification(): Promise<string[]> {
		const { data } = await this.http.get<ClaimResponse | TruverifiError>(
			"line"
		);
		...
		if (data.sms.length == 0)
			throw new Error(truverifiErrorMessage("No SMS found for this line"));
	...
	async getAccountInfo(): Promise<AccountResponse> {
		const { data } = await this.http.get<AccountResponse | TruverifiError>(
			"/account"
		);
